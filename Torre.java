...
public class Torre implements Pieza {
    ...
    public static String directorioDeImagenTorre = "src/ajedrez/torre.png"; //se pone la direccion de la imagen 
    ...
    public boolean validaMovimiento(PosicionTablero origen, PosicionTablero destino, Pieza[][] tablero) {
        return (origen.getColumna() == destino.getColumna()) || 
               (origen.getFila() == destino.getFila()); //valida las reglas del juego del rey, dice si se puede mover
    }
    ...
    public ImageIcon obtenerImagen(int ancho, int alto) { //le da las dimensiones a la imagen (la ajusta a los label)
        ...
        return new ImageIcon(new ImageIcon(directorioDeImagenTorre).getImage().getScaledInstance(ancho, alto, Image.SCALE_SMOOTH));
    }
